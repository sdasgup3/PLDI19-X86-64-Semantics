...
Consider the code snippet of the HiStar~\cite{HiStar:2006} kernel, as shown in Figure~\ref{fig:histar}(a)\footnote{\AEC{For the sake of presentation, we highlighted  (at ~\ref{fig:histar}(b)) only the key computations of the assembly compiled from the source. However, while doing the analysis, we symbolically executed the full unmodified compilation.}}, in which the KLEE~\cite{Cadar:2008} team found a security vulnerability.
