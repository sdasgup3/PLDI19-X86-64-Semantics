...
\begin{center}
    ...
    $
\end{center}
This says that the memory address $4$ stores the $0^\text{th}$ byte of the bit-vector \bv{32}{65535}, the address 5 stores the $1^\text{st}$ byte, and so on. While memory read, requested bytes are aggregated according to the size of the memory access.
...
The execution of an \ISA program begins with initialization of the configuration with the following contents of the \s{k} cell. 
\begin{lstlisting}[style=KRULE]
<k> $PGM:Instructions (*$\kra$*) fetch </k>
\end{lstlisting}
The symbol $\kra$ is used to separate the computations in the \s{k} cell and ``\s{:T}'' to represent the type of a term.
...
\s{nextloc}\footnote{\label{nextloc}For brevity, the cell \s{nextloc} is omitted from the program configuration (described in Figure~\ref{fig:config}). It is used to hold the next memory location to store an instruction. } cell. Subsequently,
...
%
 ...
 in \s{regstate}, where \s{L} is matched in \s{memstate} to find the mapped instruction.
...
lookup, appeared as  \s{getRegisterVal(\reg{rbx})}, and \s{regstate} contains a register
with name ``\s{RBX}''. This rule resolves the register lookup  to the mapped bit-vector value \s{BV$_\s{r}$} (or \s{BV$_\s{RBX}$} for the running example). 
...
...</k>
  <regstate> ... R (*$\mapsto$*) BV(*$_\s{r}$*) ... </regstate>
\end{lstlisting}
The term to the left of $\Rightarrow$ shows the memory addressing expression, of type \s{Mem}, at the 
