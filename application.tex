...
For this reason, the applications are illustrative only \AEC{and do not represent a comprehensive evaluation. Moreover, the reported runtime of individual applications are not impressive either.
...
However, we believe that each application, backed with our preliminary evaluation, has the potential to be leveraged into a \emph{practical} standalone tool, with its own user interface and case studies% and even reference manual
