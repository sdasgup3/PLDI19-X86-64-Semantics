...
|l121: "that is, 3155 instructions..." -> Unclear here whether these are the number of formalised ones or excluded ones; by cross-checking we understand that this is the number of formalised one. Maybe clarify this by giving the number of deprecated, crypto and concurrency ones respectively. |  Added the individual numbers |121 |
|l143: "It consists of 996 mnemonics" -> again, you just talked about all the deprecated instructions, but you refer here to the total; maybe you can make this clearer by giving the number of deprecated ones.| Provided an approximation of legacy and deprecated instructions |  144-148  |
|l273: "We support all but a few non-deprecated user-level instructions, amounting to 3155" -> again, the "amounting" is confusing here as it comes just after the "few non-deprecated..."|Reprased the sentence | 277 |
...
|l324: "we took the semantic rules for about 60% of the instructions in scope from the formal semantics in Strata"-> does this mean that all of the Strata instructions represent 60% of the ones you target? Or is there more in Strata which you don't reuse? | Reprased the sentence  | 342 |
...
|l629: In "(= 118 × 256)^7" the "7" is a footnote but could be interpreted as a power ;)| Addressed the ambiguity | 655|
...
|can you handle system-level (and crypto) instructions with your approach or is it an intrinsic limitation? If it's a limitation, what's the impact of not supporting it for the applicability of the work? | Already covered above |320|
...
|is the value of undef non-deterministically chosen each time, or is it the case that every undef of a specific type is always the same value of that type? (my guess is the former, but it would be nice if the text said so)|Rephrased |196-197|
|it bugs me that the definition of the Value type is not shown in the paper, or at least thoroughly described| Clarified the text on value type |409-416 & 441-444|
|for most part, I think I understand the K syntax. However, on page 5, a rule takes as input Offset of type int and on the right-hand-side it uses 64'Offset -- I guess the notation "64'" makes a 64-bit word of it. I'm also confused by the ":Mem" type annotation. What exactly is of type "Mem" here?|Clarified that it is the memry addressing expression which is if type Mem. Also, as we explicitly mentioned the value type in line 409-416, we hope that `64'Offset` notation will make sense now| 551-552|
|when you first mention the fault in the semantics of FMA, you do not explain why there is such a problem -- my guess is that it is inherited from some defect in a floating-point library of K -- please explain where the error comes from.|This is a defect in our library implementation and we clarified it.| 776-777 & 1223-1228|
|I note that there is a paper titled "Formally Verified Big Step Semantics out of x86-64 Binaries" accepted for CPP'19, which might be relevant related work.|We added the work in our related work section with relevant comparison| 1281|
...
|It was not initially clear that each instruction was validated against 7,000+ test vectors: at first I thought only 7,000 tests total were run.|We clarified this right in the itroduction section |128|
...
|Is alignment considered in memory instructions?| Clarified |footnote 3 at 425|
|What is the "coverage" of the specification itself? I.e. how much fraction of the specification is tested to have parity with hardware? The 7000 inputs for each instruction may overlook some specific input configurations for instructions, especially when the instruction has undefined output for a subset of bits.| Clarified the coverage of semantic rules |755-759|
