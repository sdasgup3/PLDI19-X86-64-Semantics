...
\begin{center}
    ...
    $
\end{center}
Here the memory address $4$ stores the $0^\text{th}$ byte of the bit-vector \bv{32}{65535}, the address 5 stores the $1^\text{st}$ byte, and so on. When memory is read, requested bytes are aggregated according to the size of the memory access.
