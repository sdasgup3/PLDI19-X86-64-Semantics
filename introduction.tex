...
We faithfully specified the semantics of the remaining instructions \Added{\emph{manually}} to obtain the complete semantics.  \Added{A manual specification may sound a daunting effort at first, but the fact that (1) \ISA is largely stable and changes slowly over time, and (2) the state-of-the-art synthesis techniques for language semantics (notably, \Strata~\cite{Heule2016a} and Hasabnis \etal~\cite{Hasabnis:ASPLOS16, Hasabnis:FSE16}) suffer from scalability (e.g., in~\cite{Heule2016a}) and/or faithfulness (e.g., in~\cite{Hasabnis:ASPLOS16, Hasabnis:FSE16}) issues (see Section~\ref{sec:Approach:Overview} and \ref{sec:RW} for details), makes the effort worth undertaking.}
